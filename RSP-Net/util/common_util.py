...
def intersectionAndUnionGPU(output, target, K, ignore_index=255):
    ...
    intersection = output[output == target] #交集部分
    area_intersection = torch.histc(intersection, bins=K, min=0, max=K-1)
    area_output = torch.histc(output, bins=K, min=0, max=K-1)
    area_target = torch.histc(target, bins=K, min=0, max=K-1)
    area_union = area_output + area_target - area_intersection #并集部分
    return area_intersection, area_union, area_target
...
def convert_to_syncbn(model): #同步批量归一化
    def recursive_set(cur_module, name, module): #递归设置
        if len(name.split('.')) > 1:
...
def smooth_loss(output, target, eps=0.1):  #平滑损失
