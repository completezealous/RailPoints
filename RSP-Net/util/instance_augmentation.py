...
class instance_augmentation(object):
    def __init__(self,instance_pkl_path,thing_list,class_weight,random_flip = False,random_add = False,random_rotate = False,local_transformation = False):
        self.thing_list = thing_list#物体类别列表
        self.instance_weight = [class_weight[thing_class_num-1] for thing_class_num in thing_list]#物体类别权重
        #self.instance_weight = np.asarray(self.instance_weight)/np.sum(self.instance_weight)#物体类别权重归一化
        self.random_flip = random_flip
        self.random_add = random_add
        self.random_rotate = random_rotate
        self.local_transformation = local_transformation

        self.add_num = 6 #每个类别添加的实例数

        with open(instance_pkl_path, 'rb') as f: #打开实例文件
            self.instance_path = pickle.load(f)#读取实例文件

    def instance_aug(self, point_xyz, point_label, point_inst=None, point_feat = None):
        ...
        if point_feat and point_inst is not None:
            return point_xyz,point_label,point_inst,point_feat
        else :
            return point_xyz,point_label

    def instance_flip(self, points,axis,center,flip_type = 1):#翻转
    ...
    def check_occlusion(self,points,center,min_dist=2):#检查遮挡
