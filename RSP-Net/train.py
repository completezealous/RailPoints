...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    args, best_iou,best_train_iou = argss, 0,0
    if args.distributed:
    ...
    if args.arch == 'unet_multi_view':
        ...
        args.patch_size = np.array([args.voxel_size[i] * args.patch_size for i in range(3)]).astype(np.float32)#将输入的 patch_size 转换为体素坐标
    ...
    args.aug=getattr(args, "aug", False)
    if args.aug:
        train_loader = torch.utils.data.DataLoader(train_data,
            batch_size=args.batch_size,
            shuffle=(train_sampler is None),
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.aug:
            loss_train, mIoU_train, mAcc_train, allAcc_train = train_tta(train_loader, model, criterion, optimizer, epoch, scaler, scheduler, gpu)
        if args.aug==False:
        ...
        if main_process():
            ...
            #is_train_iou = mIoU_train > best_train_iou
            best_train_iou=max(mIoU_train,best_train_iou)

        is_best = False
        if args.evaluate and (epoch_log % args.eval_freq == 0):
            loss_val, mIoU_val, mAcc_val, allAcc_val, terrain, rail, people, vegetation, artifaction= validate(val_loader, model, criterion)
            ...
            if main_process():
                ...
                writer.add_scalar('terrain', terrain, epoch_log)
                writer.add_scalar('rail', rail, epoch_log)
                writer.add_scalar('people', people, epoch_log)
                writer.add_scalar('vegetation', vegetation, epoch_log)
                writer.add_scalar('artifaction', artifaction, epoch_log)
    ...
    if main_process():
        writer.close()
        logger.info('==>Training done!\nBest Iou: %.4f' % (best_iou))
        logger.info('\nBest train Iou: %.4f' % (best_train_iou))
...
def train(train_loader, model, criterion, optimizer, epoch, scaler, scheduler, gpu):
    ...
    for i, batch_data in enumerate(train_loader):  # (n, 3), (n, c), (n), (b)
        ...
        coord, xyz, feat, target, offset= batch_data #coord: (n, 3), xyz: (n, 3), feat: (n, c), target: (n), offset: (b)
        ...
        coord[:, 1:] += (torch.rand(3) * 2).type_as(coord)#(n, 4)
        ...
        sinput = spconv.SparseConvTensor(feat, coord.int(), spatial_shape, args.batch_size)#(b, c, h, w, d)

        assert batch.shape[0] == feat.shape[0]

        use_amp = args.use_amp#混合精度训练
        with torch.cuda.amp.autocast(enabled=use_amp):
            output= model(sinput, xyz, batch) #得到的是一个分割分数tensor
            ...
            if loss_name == 'focal_loss':
                loss = focal_loss(output, target, criterion.weight, args.ignore_label, args.loss_gamma)#loss_gamma=2
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            ...
            if isinstance(lr, list):
                lr = [round(x, 8) for x in lr]#保留8位小数
...
def train_tta(train_loader, model, criterion, optimizer, epoch, scaler, scheduler, gpu):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    loss_meter = AverageMeter()
    intersection_meter = AverageMeter()
    ...
    for i, batch_data_list in enumerate(train_loader):  # (n, 3), (n, c), (n), (b)
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            ...
            for batch_data in batch_data_list:
                coord, xyz, feat, target, offset, inds_reconstruct = batch_data  # coord: (n, 3), xyz: (n, 3), feat: (n, c), target: (n), offset: (b)
                inds_reconstruct = inds_reconstruct.cuda(non_blocking=True)  # non_blocking=True表示异步加载数据
                # print('target',len(target))
                # print('xyz',len(xyz))
                # print('inds_reconstruct',len(inds_reconstruct))
                offset_ = offset.clone()
                offset_[1:] = offset_[1:] - offset_[:-1]  #
                batch = torch.cat([torch.tensor([ii] * o) for ii, o in enumerate(offset_)], 0).long()
                ...
                output_i = model(sinput, xyz, batch)  # 得到的是一个分割分数tensor
                output_i = output_i[inds_reconstruct, :]
                output = output + output_i;
                # print('output_i',len(output_i))
...
def validate(val_loader, model, criterion):
    if main_process():
        logger.info('>>>>>>>>>>>>>>>> Start Evaluation during traing >>>>>>>>>>>>>>>>')
    batch_time = AverageMeter()
    data_time = AverageMeter()
    loss_meter = AverageMeter()
    intersection_meter = AverageMeter()
...
def validate_tta_training(val_loader, model, criterion):
    if main_process():
        logger.info('>>>>>>>>>>>>>>>> Start Evaluation training >>>>>>>>>>>>>>>>')
...
def validate_tta(val_loader, model, criterion):
    ...
    folder_name = '/home/lk/SphereFormer/test_result/test_result_{}'.format(t)
    ...
    for i, batch_data_list in enumerate(val_loader):
        ...
        np.savetxt(os.path.join(folder_name,f'predict_label_{i+1}_{m}.txt'),predict_label,fmt='%d')
